...
class MemoLib {
  constructor (config = {}) {
    ...
    if (config.interface === 'consumer-api') {
      this.interface = 'consumer-api'
    }

    // Encapsulate dependencies
    this.bchConsumer = new BchConsumer(config)
  }
  ...
  async memoPush (msg, wif, preface = '') {
    try {
      ...
      console.log(`Encoding this in OP_RETURN: ${preface}${msg}`)
  ...
  // Same as getTransactions(), but it uses the IPFS-based infrastructure instead
  // of api.fullstack.cash.
  async getTransactions2 (bchAddr, numChunks = 1, delay = 500) {
    try {
      // Get transaction history for the address.
      const transactions = await this.bchConsumer.bch.getTransactions(bchAddr)
      // console.log(`transactions: ${JSON.stringify(transactions, null, 2)}`)
      ...
      for (let i = 0; i < txsChunks.length; i++) {
        ...
        for (let i = 0; i < txids.length; i++) {
          ...
          const tempTxData = await this.bchConsumer.bch.getTransaction(thisTxid)
          txDataResult.push(tempTxData)
        }
      ...
      console.error('Error in memo.js/getTransactions2()')
      throw err
    }
  }
