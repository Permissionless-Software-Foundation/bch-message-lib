...
class MemoLib {
    ...
    try {
      ...
      for (let i = 0; i < txData.length; i++) {
        ...
        if (msg) {
          ...
          let data = false
          if (preface) {
            const filteredMsgs = this.filterMSG(msg, preface)
            // console.log(
            //   `filteredMsgs: ${JSON.stringify(filteredMsgs, null, 2)}`
            // )

            if (filteredMsgs) {
              data = {}
              data.preface = preface
              data.subject = `${filteredMsgs.hash} ${filteredMsgs.subject}`
            }
          } else {
            data = {}
            data.subject = msg
  ...
  async memoPushOld (msg, wif, preface = '') {
    try {
      ...
      console.error('Error in memoPushOld()')
      throw err
    }
  }
  ...
  async writeMsgSignalOld (wif, ipfsHash, receivers, subject) {
    try {
      // Input Validations
      if (typeof wif !== 'string') {
        throw new Error('WIF must be a string of a private key.')
      ...
      console.error('Error in writeMsgSignalOld()')
      throw err
    }
  }
