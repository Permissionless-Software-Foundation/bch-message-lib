...
class MemoLib {
  ...
  async readMsgSignal (bchAddr, preface) {
    try {
      ...
      const txs = txsArr.reverse().slice(0, 20)
      ...
      for (let i = 0; i < txs.length; i++) {
        ...
        for (let j = 0; j < thisTx.vout.length; j++) {
          ...
          let prefaceStr = 'MSG IPFS'
          if (preface) prefaceStr = preface

          if (msg) {
            // Filter the code to see if it contains an IPFS hash And Subject.
            const data = this.filterMSG(msg, prefaceStr)
            if (data) {
              // Get sender Address
              const senderAddr = thisTx.vout[0].scriptPubKey.addresses[0]
  ...
  sortTxsByHeight (txs, sortingOrder = 'ASCENDING') {
    if (sortingOrder === 'ASCENDING') { return txs.sort((a, b) => a.height - b.height) }
