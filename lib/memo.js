...
class MemoLib {
  ...
  async getTransactions (bchAddr, numChunks = 1) {
    try {
      ...
      // Quantity of chunks to use to require 
      // the transaction details
      let txsChunks = []

      // takes all transaction chunks, if the input
      // numChunks is 0 or greater than the numChunks txs
      if (numChunks === 0 || txs.length <= numChunks) {
        txsChunks = txs
      } else {
        // just take the required chunks of the numchunks input
