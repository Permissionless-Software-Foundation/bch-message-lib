...
class MemoLib {
  ...
  async writeMsgSignal (wif, ipfsHash, receivers, subject) {
    try {
      ...
      transactionBuilder.addOutput(bchAddr, originalAmount - fee - (dust * receivers.length))
  ...
  async getTransactions (bchAddr, numChunks = 1) {
    try {
      ...
      const updatedTransactions = await this.updateNegativeHeight(transactions.transactions)
      // console.log(`updatedTransactions: ${JSON.stringify(updatedTransactions, null, 2)}`)
      ...
      const txs = this.bchUtil.util.chunk20(txsArr)
  ...
  // Update the negative value of the txs height
  // with the updated value of blockCount
  async updateNegativeHeight (txids) {
    try {
      const arr = txids
      if (!Array.isArray(txids)) throw new Error('txids must be an array')

      // Validates if there are heigth with negative values
      const hasNegativeH = txids.filter(val => val.height < 0)
      if (!hasNegativeH.length) { return arr }

      // get blockcount
      const blockCount = await this.bchjs.Blockchain.getBlockCount()

      // Update the negative height
      for (let i = 0; i < arr.length; i++) {
        if (arr[i].height <= 0) arr[i].height = blockCount
      }
      return arr
    } catch (error) {
      console.error('Error in updateNegativeHeight()')
      throw error
    }
  }
