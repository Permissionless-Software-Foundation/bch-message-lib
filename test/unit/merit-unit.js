...
describe('#merit.js', () => {
  ...
  describe('#findTokenParent', () => {
    it('should get the parent of a token UTXO', async () => {
      // Mock network calls.
      sandbox.stub(uut.bchjs.RawTransactions, 'getRawTransaction').resolves(mockData.mockTxInputs)
      sandbox.stub(uut.bchjs.Electrumx, 'transactions').resolves(mockData.mockTxHistory)
      sandbox.stub(uut.bchjs.SLP.Utils, 'hydrateUtxos').resolves(mockData.mockParentUtxo1)
  ...
  describe('#getParentAge', () => {
    it('should get the age of the oldest parent', async () => {
      // Mock network calls.
      sandbox.stub(uut, 'findTokenParent').onCall(0).resolves(mockData.mockParentUtxo2).onCall(1).resolves(false)
